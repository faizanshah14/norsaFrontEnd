...
export default function CustomizedTables(props) {
    ...
    const dormint = () => {
        ...
        alert("active clicked over there");
    ...
    const fetchMyAPI = useCallback(async () => {
        ...
        if (token) {
            try {
                ...
                NotificationManager.error('Token Expiry issue login again', 'Error!', 5000, () => {
                    // alert('Token Expired');
                });
                console.log(err);
                // history.push("/login");
                            ...
                            {data.map((row, index) => (
                                <StyledTableRow key={index}>
                                    ...
                                    <StyledTableCell align="right">
                                        <Link to={props.data.editUrl + row[props.data.clientColumns[0]]}> <Button className="edittable" variant="outlined" color="primary" size="small">
                                            <EditIcon className="editIcon" size="small" />
                                        </Button></Link>
                                        <Link to={props.data.viewUrl + row[props.data.clientColumns[0]]}> <Button className="edittable" variant="outlined" color="primary" size="small">
                                            <VisibilityIcon />
                                        </Button></Link>
                                    </StyledTableCell>
